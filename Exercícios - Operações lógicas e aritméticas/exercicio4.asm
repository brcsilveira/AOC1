...
ori $t1, $zero, 0x1234
sll $t1, $t1, 16 # cada casa ocupa 4 bits, por isso a rotação a esquerda em 16
ori $t1, $t1, 0x5678

# t3 é mascara é começa setada para pegar o digito menos significativo
ori $t3, $zero, 0xf

# t4 é carregador com o valor de t1
or $t4, $zero, $t1

# Continuar
and $t2, $t3, $t4 # É feita uma and entre t3 e t4 para pegar o bit menos significativo e salvar em t2
sll $t2, $t2, 4 # Rotação à esquerda em t2 para esperar o proximo bit.
srl $t4, $t4, 4 # Rotação à direita em t4 para que o pegar o novo bit menos significativo (7)
...
and $t4, $t4, $t3 # and entre t4 e t3 para pegar o bit menos significativo (6) e guardar no próprio t4
or $t2, $t2, $t4 # or entre t2 e t4 para guardar o o bit menos significativo em t2 (00000876)

# E assim sucessivamente até o valor de t1 estar completamente invertido em t2
